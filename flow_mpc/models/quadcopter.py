...
class QuadcopterModel(GenerativeModel):
    ...
    def __init__(self, world_dim=2, dt=0.01, kinematic=True):
        ...
        super().__init__(dynamics=dynamics, sigma=5, state_dim=state_dim, control_dim=control_dim)
        self.collision_fn = QuadcopterCollisionFcn(0.05, 0.1, world_dim)

    def goal_log_likelihood(self, state, goal, vel_penalty=0.1):
        ...
        if self.dynamic and vel_penalty is not None:
            ...
            goal_ll = goal_ll - vel_penalty * velocity_penalty
    ...
    def orientation_constraints(self, state):
        ...
        over_lim = torch.where(orientation.abs() > np.pi / 3.0, torch.ones_like(orientation),

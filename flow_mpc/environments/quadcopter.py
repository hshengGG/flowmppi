...
class QuadcopterDynamicEnv(Environment):
    ...
    def reset_start_and_goal(self, zero_velocity=False):
        ...
        while start_and_goal_in_collision:
            ...
            if self.world_type == 'stairway':
                start[0] = 1.8 * np.random.rand()
                start[1] = -1.8 * np.random.rand()

                start[2] = 0.3 * np.random.rand() - 1.6
                goal[2] = 0.1 * np.random.rand() + 1.9
                #start[0] =
                #goal[1] = 0.9 + 0.9 * np.random.rand()
                goal[0] = 3.4 * np.random.rand() - 1.4
            elif self.world_type == 'rooms2':
                #start[1] = 1.8 - 2.7 * np.random.rand()
                #goal[1] = 1.8 - 0.1 * np.random.rand()
                start[1] = np.random.randn() * 0.2 - 0.4
                goal[1] = np.random.randn() * 0.2 - 0.4
                start[0] = 1.8 - 0.4 * np.random.rand()
                goal[0] = -1.4 + 0.4 * np.random.rand()

                start[2] = np.random.randn() * 0.2 - 1
                goal[2] = np.random.randn() * 0.2 - 1
            # Angles in rad
            # Heading angle can be anything
            start[5] = -np.pi + 2 * np.pi * np.random.rand()
            goal[5] = -np.pi + 2 * np.pi * np.random.rand()
    ...
    def cost(self, vel_penalty):
        return 10 * np.linalg.norm(self.state[:self.world.dw] - self.goal[:self.world.dw]) + \
            vel_penalty * np.linalg.norm(self.state[9:])

    def __str__(self):
        return f'quadcopter_{self.world_type}_{self.world.dw}D'

    def at_goal(self):
        return np.linalg.norm(self.state[:self.world.dw] - self.goal[:self.world.dw]) < 0.3#0.1 * self.world.dw
